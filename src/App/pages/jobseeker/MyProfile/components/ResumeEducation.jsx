...
const ResumeEducation = ({ className: cx, isShowing, toggle }) => {
	...
	useEffect(() => {
		updateReset({
			...
			// redu_date: '2023-09-0610:00:00',
			// redu_date: result?.data?.redu_date,
						...
						{resumeEducation?.length > 0 ? (
							resumeEducation?.map((item) => (
								<div className={cx('item')}>
									...
									<div className={cx('content')}>
										<ul>
											<li>{item?.redu_name}</li>
											<li>{degree[item?.redu_degree]}</li>
											<li>Tốt nghiệp {formatDate(item.redu_date)}</li>
