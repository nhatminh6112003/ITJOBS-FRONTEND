...
import { useDeleteFeedBackMutation, useGetAllFeedBackQuery } from '~/App/providers/apis/feedBackApi';
...
const FeedBack = () => {
	...
	const columns = useMemo(
		() => [
			...
			{ Header: 'Tên email', accessor: 'email' },
			{
				Header: 'Lý do',
				accessor: 'reason',
				Cell: ({ row: { values } }) => {
					if (values.reason === 0) {
						return 'Việc làm không hợp pháp';
					} else if (values.reason === 1) {
						return 'Không cung cấp đủ thông tin';
					} else {
						return 'Khác';
					}
				}
			},
			{ Header: 'Link bài xấu', accessor: 'job_url' },
			{
				Header: 'Thao tác',
				accessor: 'id',
				Cell: ({ value }) => (
